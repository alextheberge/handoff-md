...
export function buildTree(
  ...
  const ignoreDirs = new Set([...IGNORE_DIRS, ...extraIgnoreDirs]);
...
export function treeToString(entries: DirectoryEntry[], prefix = ""): string {
  const lines: string[] = [];
  let count = 0;
...
export function detectConventions(cwd: string): Convention[] {
  ...
  const hasColocatedTests = files.some(
  ...
  if (
    files.some(
      (f) => f.includes("store") && (f.endsWith(".ts") || f.endsWith(".tsx") || f.endsWith(".js")),
    )
  ) {
...
function todoScanRoots(cwd: string): string[] {
  const roots: string[] = [];
  if (fs.existsSync(path.join(cwd, "src"))) roots.push(path.join(cwd, "src"));
  if (fs.existsSync(path.join(cwd, "packages"))) roots.push(path.join(cwd, "packages"));
  if (roots.length === 0) roots.push(cwd);
  return roots;
}

export function findTodos(cwd: string, maxResults = 10): Todo[] {
  const todos: Todo[] = [];
...
function adaptiveMaxDepth(cwd: string): number {
  try {
    const count = fs.readdirSync(cwd).filter((e) => !e.startsWith(".")).length;
...
export function analyzeStructure(cwd: string, options: StructureOptions = {}): StructureData {
  const maxDepth = options.maxDepth ?? adaptiveMaxDepth(cwd);
  return {
    tree: buildTree(cwd, maxDepth, options.extraIgnoreDirs ?? []),
    entryPoints: detectEntryPoints(cwd),
    conventions: detectConventions(cwd),
    todos: options.scanTodos === false ? [] : findTodos(cwd),
