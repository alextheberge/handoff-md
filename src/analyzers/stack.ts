...
function readJSON(cwd: string, file: string): PackageJson | null {
  try {
    return JSON.parse(fs.readFileSync(path.join(cwd, file), "utf-8")) as PackageJson;
  } catch {
    return null;
  }
}
...
function hasAnyDep(pkg: PackageJson | null, names: string[]): string | undefined {
  return names.find((n) => hasDep(pkg, n));
}

function setFramework(info: StackInfo, name: string, confidence: number): void {
  info.framework = name;
  info.frameworkConfidence = confidence;
}
...
export function analyzeStack(cwd: string): StackInfo {
  ...
  if (pkg?.type) info.moduleType = pkg.type;
  if (pkg?.engines) {
    info.engines = Object.entries(pkg.engines)
      .map(([k, v]) => `${k} ${v}`)
      .join(", ");
  }
  ...
  } else if (fileExists(cwd, "deno.json") || fileExists(cwd, "deno.jsonc")) {
    info.packageManager = "deno";
  }

  // Framework detection (JS/TS ecosystem)
  if (pkg) {
    if (hasDep(pkg, "next")) setFramework(info, "Next.js", 0.98);
    else if (hasDep(pkg, "nuxt")) setFramework(info, "Nuxt", 0.98);
    else if (hasDep(pkg, "@remix-run/node") || hasDep(pkg, "@remix-run/react"))
      setFramework(info, "Remix", 0.98);
    else if (hasDep(pkg, "@sveltejs/kit")) setFramework(info, "SvelteKit", 0.98);
    else if (hasDep(pkg, "svelte")) setFramework(info, "Svelte", 0.9);
    else if (hasDep(pkg, "vue")) setFramework(info, "Vue", 0.9);
    else if (hasDep(pkg, "@angular/core")) setFramework(info, "Angular", 0.98);
    else if (hasDep(pkg, "react")) setFramework(info, "React", 0.85);
    else if (hasDep(pkg, "express")) setFramework(info, "Express", 0.95);
    else if (hasDep(pkg, "fastify")) setFramework(info, "Fastify", 0.95);
    else if (hasDep(pkg, "hono")) setFramework(info, "Hono", 0.95);
    else if (hasDep(pkg, "elysia")) setFramework(info, "Elysia", 0.95);
    else if (hasDep(pkg, "astro")) setFramework(info, "Astro", 0.98);
    else if (hasDep(pkg, "gatsby")) setFramework(info, "Gatsby", 0.98);
    else if (hasDep(pkg, "electron")) setFramework(info, "Electron", 0.98);
    else if (hasDep(pkg, "react-native")) setFramework(info, "React Native", 0.98);
    else if (hasDep(pkg, "expo")) setFramework(info, "Expo", 0.98);
  }
  ...
  if (info.language === "Python") {
    try {
      ...
      if (pyproject.includes("django")) setFramework(info, "Django", 0.9);
      else if (pyproject.includes("fastapi")) setFramework(info, "FastAPI", 0.9);
      else if (pyproject.includes("flask")) setFramework(info, "Flask", 0.9);
    } catch {}
    try {
      const reqs = fs.readFileSync(path.join(cwd, "requirements.txt"), "utf-8");
      if (reqs.includes("django") || reqs.includes("Django")) setFramework(info, "Django", 0.85);
      else if (reqs.includes("fastapi")) setFramework(info, "FastAPI", 0.85);
      else if (reqs.includes("flask") || reqs.includes("Flask")) setFramework(info, "Flask", 0.85);
    } catch {}
  }
  ...
  if (info.language === "Rust") {
    try {
      ...
      if (cargo.includes("actix-web")) setFramework(info, "Actix Web", 0.95);
      else if (cargo.includes("axum")) setFramework(info, "Axum", 0.95);
      else if (cargo.includes("tauri")) setFramework(info, "Tauri", 0.95);
      else if (cargo.includes("rocket")) setFramework(info, "Rocket", 0.95);
    } catch {}
  }

  // Go frameworks (module path checks to avoid false positives)
  if (info.language === "Go") {
    try {
      const gomod = fs.readFileSync(path.join(cwd, "go.mod"), "utf-8");
      if (/gin-gonic\/gin/.test(gomod)) setFramework(info, "Gin", 0.95);
      else if (/gofiber\/fiber/.test(gomod)) setFramework(info, "Fiber", 0.95);
      else if (/labstack\/echo/.test(gomod)) setFramework(info, "Echo", 0.95);
    } catch {}
  }
  ...
  else if (fileExists(cwd, "railway.json") || fileExists(cwd, "railway.toml"))
    info.deploy = "Railway";
  else if (
